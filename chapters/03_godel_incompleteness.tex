...
\begin{question}Can mathematics prove every truth? Can a formal system prove its own consistency?\end{question}

Hilbert believed the answer was yes. He famously declared in 1930: \emph{``Wir müssen wissen, wir werden wissen''} (``We must know, we shall know''). Gödel proved the answer is \emph{no} --- twice.

\begin{highlightbox}[The Core Question]Is there a formal system that captures all mathematical truth? Or are there true statements that can never be proven within the system? And can a system prove its own consistency?\end{highlightbox}

The second of Hilbert's famous 23 problems (1900) asked for a proof of the consistency of the arithmetic axioms. Hilbert believed that such a proof would forever secure mathematics against paradox. When he retired in 1930, his address to the Society of German Scientists and Physicians reiterated his unwavering faith: no mathematical truth would remain unknown. Within a year, a 25-year-old Austrian logician had shattered this vision.

%======================================================================
% SECTION 2: WHY IT SEEMED IMPOSSIBLE
%======================================================================
\section{Why It Seemed Impossible}
\label{sec:godel:impossible}
...
\item \textbf{Self-Reference as Paradox:} The liar paradox (``This statement is false'') shows that self-referential statements break logic. Gödel's insight was to \emph{harness} self-reference, not avoid it. But before Gödel, self-reference in formal systems was considered a bug to be eliminated (Russell's type theory), not a tool.
\end{enumerate}

\begin{roadblockbox}[The Liar Paradox Barrier]
``This statement is false.'' If true, it's false. If false, it's true. Classical logic bans such statements. Russell's type theory (1908) stratified propositions to prevent self-reference. Tarski (1933) later showed that a language cannot define its own truth predicate without paradox. Gödel had to \emph{encode} self-reference in arithmetic --- where no one expected it to live.
...
\label{sec:godel:context}

\begin{historicalbox}[Hilbert's Program (1920s)]
David Hilbert, the most influential mathematician of his era, proposed: formalize mathematics completely, then prove the formal system consistent using only finitary reasoning (reasoning about concrete symbols, not infinite sets). This would secure mathematics against paradoxes forever. The program had two phases: \emph{axiomatization} (express all mathematics in formal language) and \emph{consistency proving} (use finitary metamathematics to show no contradiction can be derived).

\end{historicalbox}

Key precursors to Gödel's breakthrough:
...
Gödel numbering (encoding syntax as arithmetic) is the direct predecessor of Turing's encoding of machines as data. Both rely on: \emph{syntax can be made into data}. Turing read Gödel's 1931 paper and used the same technique for his 1936 paper. The stored-program computer (von Neumann architecture) is a physical realization of this principle: programs are data.
...
\begin{mentalLeap}[The Insight]Statements \emph{about} a formal system (provability, consistency) can be translated into statements \emph{within} the system (arithmetic) by encoding symbols, formulas, and proofs as numbers. The statement ``Formula $\phi$ is provable'' becomes an arithmetic statement about the Gödel number of $\phi$. Then, using diagonalization, construct a sentence $G$ that says ``I am not provable.'' If $G$ is provable, the system is inconsistent. If $G$ is not provable, $G$ is true but unprovable --- the system is incomplete.\end{mentalLeap}
...
\begin{roadblockbox}[Why This Was Shocking]
...
\subsubsection*{The Provability Predicate}
...
Before his incompleteness results, Gödel proved a \emph{completeness} theorem: for first-order logic, every semantically valid formula is provable. Formally, if $\models \phi$ (every model satisfies $\phi$), then $\vdash \phi$ (there is a formal proof). This was the first major result connecting semantics (truth in models) with syntax (formal provability) for first-order logic.

The completeness theorem seems to contradict the incompleteness theorem, but there is no contradiction: completeness applies to first-order \emph{logic} (the deductive apparatus), while incompleteness applies to specific \emph{theories} (like PA). The logical rules are complete: everything that is logically valid is provable. But the axioms of PA do not capture all arithmetic truths --- the set of models of PA includes non-standard models (with infinite numbers) where different sentences are true.
...
\begin{highlightbox}[Löb's Theorem Intuition]
...
\subsection{Tarski's Undefinability of Truth (1933)}
\label{sec:godel:tarski}

Closely related to Gödel's theorems is Tarski's result that a sufficiently expressive language cannot define its own truth predicate.
...
Tarski's theorem is the formal version of the liar paradox: ``This statement is false'' cannot be expressed within the language without contradiction. While Gödel showed that \emph{provability} is expressible, Tarski showed that \emph{truth} (in the standard model) is not. This hierarchy: truth is not definable in the object language but can be defined in a meta-language (Tarski's hierarchy of languages).

Note the contrast: $\mathsf{Prov}$ is a $\Sigma_1$ formula that captures provability, but it does not capture truth --- there are true but unprovable sentences. Truth is a $\Pi_1^1$ notion (truth in the standard model requires quantifying over all subsets of $\N$), far beyond the expressive power of a $\Sigma_1$ formula.
...
\begin{highlightbox}[Gödel's Own Philosophy]
...
%======================================================================
\section{Citations}
\label{sec:godel:citations}

\begin{enumerate}
  \item \cite{godel1931}
  \item \cite{godel1931b}
  \item \cite{rosser1936}
  \item \cite{gentzen1936}
  \item \cite{cohen1963}
  \item \cite{smorynski1977}
  \item \cite{boolos1993}
  \item \cite{franzen2005}
  \item \cite{smith2007}
  \item \cite{chaitin1975}
  \item \cite{paris1977}
  \item \cite{kirby1982}
  \item \cite{penrose1989}
  \item \cite{feferman1960}
\end{enumerate}
