...
\begin{question}What makes a problem \emph{inherently} difficult? Can we classify problems by their intrinsic computational cost, independent of algorithms or hardware?\end{question}
...
\begin{roadblockbox}[The Classification Problem]
...
\begin{connectionbox}[Connection to Chapter 7]
...
\begin{historicalbox}[The Pre-History of Lower Bounds]
...
\begin{roadblockbox}[The Lower Bound Barrier]
...
\begin{roadblockbox}[The Relativization Barrier]
...
\begin{historicalbox}[The Birth of Complexity Theory (1960s)]
\begin{itemize}
...
\begin{historicalnoteMacro}[Juris Hartmanis (1928--)]
...
\end{historicalnoteMacro}

\begin{historicalnoteMacro}[Richard Stearns (1936--)]
...
\begin{historicalnoteMacro}[Jack Edmonds (1934--)]
...
\begin{historicalnoteMacro}[Alan Cobham (1927--2011)]
...
\begin{mentalLeap}[The Insight]\emph{Polynomial time} ($n^{O(1)}$) is the correct mathematical formalization of ``tractable,'' ``feasible,'' ``efficiently computable.''\end{mentalLeap}
...
\begin{mentalLeap}[The Insight]\emph{Reduction} is the right tool for comparing difficulty. If problem A reduces to problem B (A $\le_p$ B), then B is ``at least as hard as'' A. This creates a \emph{partial order} on problems. The \emph{maximal} elements of NP under this order are the \emph{NP-complete} problems — the ``hardest'' problems in NP. If \emph{any} NP-complete problem is in P, then P = NP.\end{mentalLeap}
...
\begin{historicalbox}[The Invariance Thesis]
...
\begin{roadblockbox}[Implications of Natural Proofs]
...
\begin{connectionbox}[Hardness vs. Randomness]
...
\begin{connectionbox}[Quantum Supremacy]
