...
\begin{connectionbox}[Connection to Chapter 9 (NP-Completeness)]
...
\begin{connectionbox}[Connection to Chapter 8 (Computational Complexity)]
...
\begin{connectionbox}[Connection to Chapter 11 (Zero Knowledge)]
...
\begin{roadblockbox}[The Complexity Theory Barrier]
In 1985, the polynomial hierarchy (PH) was believed to be the ``top'' of the verification landscape. NP was the first level, co-NP the second, and higher levels alternated quantifiers. The question was whether anything lay \emph{above} PH. Interactive proofs showed that the hierarchy collapses: IP contains the entire polynomial hierarchy (AM = NP $\cap$ coAM at the second level, and more generally, the Arthur-Merlin hierarchy collapses to a finite number of levels). This was unexpected: interaction was more powerful than alternation.
\end{roadblockbox}
...
\begin{historicalbox}[The Pre-1985 Landscape]
...
The \emph{polynomial hierarchy} (PH) was \cite{stockmeyer1976}, defined by Megiddo, Maseih, and Tiwari (1981) and formalized by Stockmeyer (1977):
\begin{itemize}
\item $\Sigma_0^P = P$
\item $\Sigma_1^P = NP$
...
\item \textbf{Babai and Moran (1988):} Showed that IP is contained in PSPACE, completing the upper bound.
...
\item \textbf{László Babai (b. 1950):} Hungarian-American computer scientist. Graph isomorphism in quasipolynomial time (2015). Co-proved MIP = NEXP, IP = PSPACE. Pioneer of interactive proofs and probabilistically checkable proofs.
\end{itemize}
\end{subsection}
...
\begin{mentalLeap}[Leap 1 (GMR 1985): Interaction Is a Computational Resource]
...
\begin{mentalLeap}[Leap 2 (Goldwasser-Sipser 1986): Public Coins Are Nearly as Powerful]
...
Result: $AM \subseteq NP \cap coAM$ at the second level. For any constant $k \ge 2$, $AM[k] = AM[2]$. The hierarchy of public-coin protocols collapses to two rounds. This was surprising: private coins can be strictly more powerful for some tasks, but public coins capture essentially all the power for verification.
...
\begin{mentalLeap}[Leap 3 (Shamir 1992): IP = PSPACE]
...
\begin{roadblockbox}[Why IP = PSPACE Was Surprising]
...
\begin{theorem}[AM $\subseteq$ NP $\cap$ coNP]
The class $AM$ (two-round public-coin interactive proofs) is contained in $NP \cap coNP$.

Proof: $x \in AM$ iff there exists a prover message $m$ such that the verifier accepts with probability $\ge 2/3$. This is an NP condition. $x \notin AM$ iff for all prover messages, the verifier rejects with probability $\ge 2/3$. This is a coNP condition.
\end{theorem}
...
\item \textbf{Multi-Prover Interactive Proofs (MIP):} Goldwasser, Sipser, and Ben-Or and Guruswami showed that if the prover consists of multiple non-communicating provers, the power increases dramatically: MIP = NEXP. This result was a key stepping stone to IP = PSPACE and to the PCP theorem.
\end{enumerate}
...
\item Open problems include understanding the power of quantum interactive proofs ($\mathsf{QIP} = \mathsf{PSPACE}$, Ji 2012), the relationship between interactive proofs and machine learning verification, and the practical efficiency of interactive proof systems for delegation of computation.
\end{itemize}
...
\begin{exercise}[Intermediate]
Prove that AM $\subseteq$ NP $\cap$ coNP. Show that if the polynomial hierarchy collapses to the second level, then AM = NP.
\end{exercise}
...
\item \textbf{1975:} Lamport introduces the Byzantine generals problem. Interaction in distributed systems is recognized as fundamental.
\item \textbf{1981:} Megiddo, Maseih, Tiwari define the polynomial hierarchy. Alternation of quantifiers is recognized as a computational resource.
...
\item \textbf{1990:} Fortnow shows that AM $\neq$ NP unless the polynomial hierarchy collapses.
...
\item \textbf{2008:} Gentry constructs the first fully homomorphic encryption scheme, using interactive proof techniques.
\item \textbf{2016:} Ben-Sasson, Chiesa, Gabizon, Wietersen, and Zahur introduce STARKs (Scalable Transparent ARguments of Knowledge), a practical zero-knowledge proof system based on interactive proofs.
\item \textbf{2019:} Ben-Sasson et al. introduce HyperPlonk, a universal and transparent setup-free SNARK.
...
%======================================================================
\section{Citations}
\label{sec:ip:citations}

\begin{itemize}
\item \cite{GoldwasserMicaliRackoff1985} --- The foundational paper defining interactive proofs and zero-knowledge.
\item \cite{Shamir1992} --- The IP = PSPACE theorem.
\item \cite{Babai1985} --- The Arthur-Merlin model.
\item \cite{GoldwasserSipser1986} --- Private vs. public coins.
\item \cite{FiatShamir1986} --- The Fiat-Shamir transform.
\item \cite{AroraSafra1992} --- The PCP theorem (one of two proofs).
\item \cite{AroraEtAl1992} --- The PCP theorem (the other proof).
\item \cite{BabaiMoran1988} --- IP $\subseteq$ PSPACE.
\item \cite{Fortnow1990} --- AM $\neq$ NP unless PH collapses.
\item \cite{BenSassonEtAl2016} --- STARKs.
\item \cite{BenSassonEtAl2019} --- HyperPlonk.
\end{itemize}
