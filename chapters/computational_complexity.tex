...
The distinction between polynomial and exponential growth is stark. A problem solvable in $n^3$ steps on a $10^9$ ops/sec machine can handle inputs of size $10^3$ within a second ($10^9/10^9 = 1$); even $10^4$ takes $10^{12}$ steps, i.e. $10^3$ seconds, and $10^6$ would take $10^{18}$ steps --- about 31.7 years. An exponential $2^n$ algorithm struggles at $n = 50$. This gap defines the boundary between the feasible and the intractable.
...
The invariance thesis states that all ``reasonable'' sequential models (RAM machines, pointer machines, $\lambda$-calculus, cellular automata) simulate each other with polynomial overhead. For example, Hennie and Stearns (1966) showed that any $k$-tape Turing machine can be simulated by a two-tape machine in $O(n \log n)$ time, and RAM machines simulate Turing machines with polynomial overhead (Pippenger and Fischer 1979 refined the associated time--reversal tradeoffs). This robustness justifies P as a model-independent class.
