...
\emph{Byzantine Agreement is Possible if $n > 3t$.}
...
\emph{Paxos — Leaders and Quorums.}
...
\emph{Probabilistic Consensus in Open Membership.}
...
\emph{Failure Detectors Circumvent FLP.}
...
\emph{DAG-based Consensus Separates Dissemination from Ordering.}
...
\item \textbf{Ethereum Gasper:} LMD-GHOST (fork choice) + Casper FFG (finality). Two consensus layers. Sync committee for light clients. Gasper separates block proposal (GHOST) from finality (Casper), enabling fast optimistic confirmation with finality about two epochs (roughly 12.8 minutes) after a block is proposed, with each epoch comprising 32 slots.
