...
\begin{itemize}
\item \textbf{1974:} Ralph Merkle (Berkeley undergrad) proposes \emph{public-key cryptography} in a class project (rejected initially). Publishes 1978 \cite{Merkle1978}.
\item \textbf{1976:} Whitfield Diffie \& Martin Hellman (Stanford): ``New Directions in Cryptography'' --- public-key key exchange (Diffie-Hellman), digital signatures, one-way functions \cite{DiffieHellman1976}.
\item \textbf{1977:} Rivest, Shamir, Adleman (MIT): RSA --- first practical public-key encryption + signatures \cite{RSA1978}.
...
\item \textbf{1985:} Koblitz, Miller: Elliptic Curve Cryptography (ECC) --- smaller keys, faster \cite{Koblitz1987,Miller1985}.
\item \textbf{1988:} ElGamal: Encryption variant of DH \cite{ElGamal1985}.
\item \textbf{2000s:} Pairing-based crypto (Boneh-Franklin IBE, BLS signatures) \cite{BonehFranklin2001,BonehLynnShacham2001}.
\item \textbf{2010s:} Post-quantum crypto (lattices, isogenies, codes, hashes) \cite{NISTPQC2024}.
\end{itemize}
...
\begin{itemize}
\item \textbf{Diffie \& Hellman:} Visionaries who saw the \emph{concept} (key exchange, signatures) \cite{DiffieHellman1976}.
\item \textbf{Rivest, Shamir, Adleman:} Found the \emph{trapdoor} (factoring) \cite{RSA1978}.
\item \textbf{Merkle:} Invented Merkle puzzles (pre-DH key exchange) and Merkle trees \cite{Merkle1978}.
\item \textbf{GCHQ team:} Did it first, in secret. Ellis (concept), Cocks (RSA), Williamson (DH).
\item \textbf{Koblitz \& Miller:} Independently proposed elliptic curve groups for crypto (1985) \cite{Koblitz1987,Miller1985}.
\end{itemize}
