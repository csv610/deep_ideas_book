...
1948: ``A Mathematical Theory of Communication'' --- the founding paper of information theory \cite{Shannon1948}. Vannevar Bush described it as ``one of the greatest works of the century.''
...
\item \textbf{1948:} Shannon's two-part paper in \emph{Bell System Technical Journal} --- 55 pages that created a field. It appeared in July and October 1948. By 1949, Warren Weaver had popularized it in a book version that added the famous diagram: Source $\to$ Transmitter $\to$ Channel $\to$ Receiver $\to$ Destination \cite{Shannon1949}.
\end{itemize}

Standard references: Cover and Thomas \cite{coverthomas2006}, MacKay \cite{MacKay1999}, and the Kolmogorov complexity perspective \cite{kolmogorov1965}.
...
\begin{enumerate}
\item \textbf{Meaning Is Explicitly Discarded:} ``The semantic aspects of communication are irrelevant to the engineering problem.'' (Shannon 1948 \cite{Shannon1948}) A random string and a Shakespeare sonnet of the same length have the \emph{same} information content if drawn from the same distribution. This was the most controversial move --- and the most productive.
