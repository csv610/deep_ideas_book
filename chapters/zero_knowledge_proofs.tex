...
How can you prove you know something without revealing it?
...
\begin{itemize}
\item \textbf{1982:} Goldwasser, Micali, Rackoff (GMR): ``The Knowledge Complexity of Interactive Proof Systems'' (STOC). Defined IP, ZK, knowledge complexity. Quadratic Residuosity ZK proof \cite{GoldwasserMicaliRackoff1985}.
\item \textbf{1985:} Goldreich, Micali, Wigderson (GMW): ``Proofs that Yield Nothing But Their Validity'' --- \emph{every} NP statement has a ZK proof (assuming OWFs). Graph 3-coloring ZK proof \cite{GoldreichMicaliWigderson1986}.
\item \textbf{1986:} Feige, Fiat, Shamir: ZK identification (Fiat-Shamir heuristic) \cite{FeigeFiatShamir1987,FiatShamir1986}.
\item \textbf{1988:} Ben-Or, Goldwasser, Kilian, Wigderson: Multi-prover ZK (MIP) \cite{benor1983,GoldwasserMicaliRackoff1985}.
\item \textbf{1990:} Babai, Fortnow, Lund: $\mathsf{IP} = \mathsf{PSPACE}$ (Shamir 1992) \cite{BabaiFortnowLund1991,Shamir1992}.
\item \textbf{1991:} Kilian: ZK arguments (computationally sound, succinct). PCPs \cite{Kilian1992,goldreichmicaliwigderson1991}.
\item \textbf{1992:} Feige, Kilian: Parallel repetition \cite{Feige1998}.
\item \textbf{2000s:} zk-SNARKs (Groth, Gennaro, Pinocchio, Groth16) \cite{Groth2010}, zk-STARKs (Ben-Sasson et al. 2018) \cite{BenSassonEtAl2016,BenSassonEtAl2019}, Bulletproofs (B\"unz et al. 2018) \cite{Bunz2018}.
\end{itemize}
...
\emph{Proofs Can Be Interactive and Probabilistic.}
...
\emph{Zero Knowledge = Simulatability.}
...
\emph{Every NP Statement Has a ZK Proof.}
...
\emph{Succinct Non-Interactive ZK (zk-SNARKs).}
...
The security proof relies on the \emph{Forking Lemma} (Pointcheval and Stern, 1996), formalized below: rewinding the adversary with different oracle answers yields two forgeries from which a witness can be extracted.

The Fiat-Shamir transform is \emph{sound in the Random Oracle Model} (ROM, discussed below) but not necessarily in the standard model. There exist contrived protocols (e.g., the Goldwasser-Kalai example) where the transform produces an unsound non-interactive proof despite the interactive version being sound. Nevertheless, the transform is used ubiquitously in practice (Schnorr signatures, EdDSA, zk-SNARKs, STARKs).
...
\hline
1988 & Ben-Or-Goldwasser-Kilian-Wigderson: Multi-prover ZK (MIP) \\
\hline
1990 & Babai-Fortnow-Lund: $\mathsf{MIP} = \mathsf{NEXP}$ \\
\hline
1990 & Shamir: $\mathsf{IP} = \mathsf{PSPACE}$ \\
\hline
